...
void hr4988_init(void){
  ...
  GPIO_WriteLow(STEPPER_GPIO, STEPPER_MS1 | STEPPER_MS2 | STEPPER_MS3 | STEPPER_ENA);
  GPIO_WriteHigh(STEPPER_GPIO, STEPPER_SLP);
...
void TIM2_INT_Init(void) {
  ...
  TIM2_TimeBaseInit(TIM2_PRESCALER_16, 49);
...
void TIM1_PWM_Init(uint16_t frequency) {
    ...
    TIM1_OC2PreloadConfig(ENABLE); // Habilitar pre-carga para CCR2
...
void setSpeed(uint16_t speed){
  TIM1_SetFrequency((speed/stepsPerRevolution)*60);
}
