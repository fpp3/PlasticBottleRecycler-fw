...
// First item of EVERY menu is going to work as a "button". No sub-menues there
const char **menu0[] = {submenu0_0, submenu0_1, submenu0_2, submenu0_3, submenu0_4, NULL};
// Repeating submenues is possible by repeating them in their respective menu
const char **menu1[] = {submenu1_0, submenu1_1, submenu0_1, submenu1_3, submenu0_4, NULL};
...
const buzzerTone_s bootMelody[] = {{.period = 50, .frequency = NOTE_A4, .volume = 10},
                                   {.period = 50, .frequency = NOTE_A4, .volume = 0},
                                   {.period = 75, .frequency = NOTE_A6, .volume = 10},
                                   {.period = 75, .frequency = NOTE_A6, .volume = 0},
                                   {.period = 75, .frequency = NOTE_A6, .volume = 10},
                                   {.period = 0, .frequency = 0, .volume = 0}

};
const buzzerTone_s runoutMeoldy[] = {{.period = 75, .frequency = NOTE_A6, .volume = 5},
                                     {.period = 75, .frequency = NOTE_A6, .volume = 0},
...
int8_t button_getEdge(volatile bool newState, volatile bool *currentState, volatile bool *previousState,
                      volatile uint8_t *holdCount);
size_t count_items(const char **submenu);
size_t count_submenus(const char ***menu_level);
bool menu_value_selector(char *buffer, const char *string, const char *unit, const uint16_t lowerLimit,
                         const uint16_t upperLimit, const char *numMask[], uint16_t *variable,
                         volatile int8_t *minusEdge, volatile int8_t *plusEdge, volatile int8_t *okEdge);
void save_to_EEPROM(void *value, uint8_t size, uint8_t address);
void submenu_hotend(char *buff, uint8_t *currPos, uint8_t *currSubMenu, bool *selected, uint16_t *setTemp,
                    volatile int8_t *minusEdge, volatile int8_t *plusEdge, volatile int8_t *okEdge, bool running);
void submenu_stepper(char *buff, uint8_t *currPos, uint8_t *currSubMenu, bool *selected, uint16_t *setSpeed,
                     uint16_t *uStepping, uint16_t *rotation, bool *stepper, volatile int8_t *minusEdge,
                     volatile int8_t *plusEdge, volatile int8_t *okEdge, bool running);
void submenu_sound(char *buff, uint8_t *currPos, uint8_t *currSubMenu, bool *selected, uint16_t *clickSound,
                   uint16_t *runoutNotif, uint16_t *bootSound, volatile int8_t *minusEdge, volatile int8_t *plusEdge,
                   volatile int8_t *okEdge);
void submenu_fan(char *buff, uint8_t *currPos, uint8_t *currSubMenu, bool *selected, uint16_t *fanTiedTo,
                 uint16_t *fanSpeed, volatile int8_t *minusEdge, volatile int8_t *plusEdge, volatile int8_t *okEdge);
...
void main(void) { // NOLINT
  ...
  uint16_t uStepping =
      FLASH_ReadByte(EEPROM_BASE + EEPROM_USTEPPING) << 8 | FLASH_ReadByte(EEPROM_BASE + EEPROM_USTEPPING + 1);
  uint16_t rotation =
      FLASH_ReadByte(EEPROM_BASE + EEPROM_ROTATION) << 8 | FLASH_ReadByte(EEPROM_BASE + EEPROM_ROTATION + 1);
  uint16_t bootSound =
      FLASH_ReadByte(EEPROM_BASE + EEPROM_BOOT_SOUND) << 8 | FLASH_ReadByte(EEPROM_BASE + EEPROM_BOOT_SOUND + 1);
  uint16_t clickSound =
      FLASH_ReadByte(EEPROM_BASE + EEPROM_CLICK_SOUND) << 8 | FLASH_ReadByte(EEPROM_BASE + EEPROM_CLICK_SOUND + 1);
  uint16_t runoutNotif =
      FLASH_ReadByte(EEPROM_BASE + EEPROM_RUNOUT_SOUND) << 8 | FLASH_ReadByte(EEPROM_BASE + EEPROM_RUNOUT_SOUND + 1);
  uint16_t fanTieTo =
      FLASH_ReadByte(EEPROM_BASE + EEPROM_FAN_TIE) << 8 | FLASH_ReadByte(EEPROM_BASE + EEPROM_FAN_TIE + 1);
  uint16_t fanSpeed =
      FLASH_ReadByte(EEPROM_BASE + EEPROM_FAN_SPEED) << 8 | FLASH_ReadByte(EEPROM_BASE + EEPROM_FAN_SPEED + 1);
  if (bootSound)
    ...
    if (selected == 0) {
      ...
      case 0:
        ...
        case 1:
          ...
          break;
        case 2:
          submenu_stepper(buff, &currPos, &currSubMenu, &selected, &setSpeed, &uStepping, &rotation, &stepper,
                          &minusEdge, &plusEdge, &okEdge, 0);
          break;
        case 3:
          submenu_sound(buff, &currPos, &currSubMenu, &selected, &clickSound, &runoutNotif, &bootSound, &minusEdge,
                        &plusEdge, &okEdge);
          break;
        case 4:
...
bool menu_value_selector(char *buffer, const char *string, const char *unit, const uint16_t lowerLimit,
  ...
  if (*minusEdge > 0) {
    if (*variable > lowerLimit) {
      if (*minusEdge < 3)
        (*variable)--;
      else if ((*variable - 10) > lowerLimit)
        (*variable) -= 10;
    }
  }
  if (*plusEdge > 0) {
    if (*variable < upperLimit) {
      if (*plusEdge < 3)
        (*variable)++;
      else if ((*variable + 10) < upperLimit)
        (*variable) += 10;
    }
  }
  if (*okEdge == 1)
...
void submenu_hotend(char *buff, uint8_t *currPos, uint8_t *currSubMenu, bool *selected, uint16_t *setTemp,
  ...
  switch (*currPos + 1) {
  case 1:
    *selected = menu_value_selector(buff, "Temp.:", "\322C", TEMP_LOWER_LIMIT, TEMP_UPPER_LIMIT, NULL, setTemp,
                                    minusEdge, plusEdge, okEdge);
    break;
  case 2:
    // go back
    *selected = 0;
...
void submenu_stepper(char *buff, uint8_t *currPos, uint8_t *currSubMenu, bool *selected, uint16_t *setSpeed,
  ...
  switch (*currPos + 1) {
  case 1:
    *selected = menu_value_selector(buff, "Veloc.:", "RPM", STEPPER_LOWER_LIMIT, STEPPER_UPPER_LIMIT, NULL, setSpeed,
                                    minusEdge, plusEdge, okEdge);
    if (*selected == 0)
  ...
  case 2:
    *selected =
        menu_value_selector(buff, "\371Stepping:", NULL, 0, 4, uSteps_numMask, uStepping, minusEdge, plusEdge, okEdge);
    if (*selected == 0)
  ...
  case 4:
    if (running) {
      ...
      *selected =
          menu_value_selector(buff, "Stepper:", NULL, 0, 1, state_numMask, &stepperBuf, minusEdge, plusEdge, okEdge);
    ...
    // break;  // <------ CAREFUL ------ this is so i dont need to make two switches depending on the state of 'running'
    // in order to have always the "back" at the end of the list
  case 5:
    // go back
    *selected = 0;
    *currPos = *currSubMenu;
...
void submenu_sound(char *buff, uint8_t *currPos, uint8_t *currSubMenu, bool *selected, uint16_t *clickSound,
  ...
  case 1:
    *selected =
        menu_value_selector(buff, "Notific.:", 0, 0, 1, state_numMask, runoutNotif, minusEdge, plusEdge, okEdge);
    if (*selected == 1)
...
void submenu_fan(char *buff, uint8_t *currPos, uint8_t *currSubMenu, bool *selected, uint16_t *fanTiedTo,
  ...
  case 1:
    *selected =
        menu_value_selector(buff, "Arr. con:", NULL, 0, 3, fanTie_numMask, fanTiedTo, minusEdge, plusEdge, okEdge);
    if (*selected == 1)
